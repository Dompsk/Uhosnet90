<template>

    <section id="uhosnet_group" class="pt-20 pb-20">
        <header class="pb-10">
            <p class="text-center text-[16px] sm:text-[18px] text-[#02a4a9] font-bold">UHOSNET GROUPS</p>
            <h1 class="text-center text-[28px] sm:text-[36px] font-bold text-[#003d7e] px-4">กลุ่มงานภายในเครือข่าย</h1>
            <div class="w-[250px] h-[4px] bg-amber-400 mx-auto"></div>
        </header>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center justify-items-center px-4 max-w-[1200px] mx-auto my-[30px] gap-6 sm:gap-8 lg:gap-[30px]">
            <div class="bg-white w-full max-w-[380px] sm:w-[380px] h-[200px] rounded-[1rem] shadow-[0_2px_16px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-[0_6px_24px_rgba(40,74,126,0.15)]">
                <div class="logo w-[52px] h-[52px] bg-[#19528f] rounded-[0.75rem] mb-4 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#e3e3e3"><path d="M80-120v-720h400v160h400v560H80Zm80-80h80v-80h-80v80Zm0-160h80v-80h-80v80Zm0-160h80v-80h-80v80Zm0-160h80v-80h-80v80Zm160 480h80v-80h-80v80Zm0-160h80v-80h-80v80Zm0-160h80v-80h-80v80Zm0-160h80v-80h-80v80Zm160 480h320v-400H480v80h80v80h-80v80h80v80h-80v80Zm160-240v-80h80v80h-80Zm0 160v-80h80v80h-80Z"/></svg>
                </div>
                <p class="text-[#003d7e] text-[1.8rem] font-bold">อำนวยการเครือข่าย</p>
            </div>
            <div class="bg-white w-full max-w-[380px] sm:w-[380px] h-[200px] rounded-[1rem] shadow-[0_2px_16px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-[0_6px_24px_rgba(40,74,126,0.15)]">
                <div class="logo w-[52px] h-[52px] bg-[#19528f] rounded-[0.75rem] mb-4 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#e3e3e3"><path d="M694-120.67 552.67-262l47-46.67L694-215l179.33-178.67L920-346 694-120.67Zm-154 40q-110.67 0-185.33-77.66Q280-236 280-344.67v-31q-85.33-12-142.67-77.16Q80-518 80-606.67V-840h120v-40h66.67v146.67H200v-40h-53.33v166.66q0 69.34 48.66 118Q244-440 313.33-440q69.34 0 118-48.67 48.67-48.66 48.67-118v-166.66h-53.33v40H360V-880h66.67v40h120v233.33q0 88.67-57.34 153.84-57.33 65.16-142.66 77.16v31q0 81.67 55.16 139.5Q457-147.33 540-147.33v66.66Z"/></svg>
                </div>
                <p class="text-[#003d7e] text-[1.8rem] font-bold">พยาบาลเครือข่าย</p>
            </div>
            <div class="bg-white w-full max-w-[380px] sm:w-[380px] h-[200px] rounded-[1rem] shadow-[0_2px_16px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-[0_6px_24px_rgba(40,74,126,0.15)]">
                <div class="logo w-[52px] h-[52px] bg-[#19528f] rounded-[0.75rem] mb-4 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#e3e3e3"><path d="M345-120q-94 0-159.5-65.5T120-345q0-45 17-86t49-73l270-270q32-32 73-49t86-17q94 0 159.5 65.5T840-615q0 45-17 86t-49 73L504-186q-32 32-73 49t-86 17Zm270.67-272 111.66-110.67q22-22 34-51.33t12-61q0-66-46.16-112.17Q681-773.33 615-773.33q-31.67 0-61 12t-51.33 34L392-615.67 615.67-392ZM345-186.67q31 0 60.67-12 29.66-12 51.66-34L568-344.33 344.33-568 232.67-457.33q-22 22-34 51.33t-12 61q0 66 46.16 112.17Q279-186.67 345-186.67Z"/></svg>
                </div>
                <p class="text-[#003d7e] text-[1.8rem] font-bold">เภสัชกรรมเครือข่าย</p>
            </div>
            <div class="bg-white w-full max-w-[380px] sm:w-[380px] h-[200px] rounded-[1rem] shadow-[0_2px_16px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-[0_6px_24px_rgba(40,74,126,0.15)]">
                <div class="logo w-[52px] h-[52px] bg-[#19528f] rounded-[0.75rem] mb-4 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#e3e3e3"><path d="M653.33-160v-280H800v280H653.33Zm-246.66 0v-640h146.66v640H406.67ZM160-160v-440h146.67v440H160Z"/></svg>
                </div>
                <p class="text-[#003d7e] text-[1.8rem] font-bold">คุณภาพเครือข่าย</p>
            </div>
            <div class="bg-white w-full max-w-[380px] sm:w-[380px] h-[200px] rounded-[1rem] shadow-[0_2px_16px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-[0_6px_24px_rgba(40,74,126,0.15)]">
                <div class="logo w-[52px] h-[52px] bg-[#19528f] rounded-[0.75rem] mb-4 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#e3e3e3"><path d="M38.67-160v-100q0-34.67 17.83-63.17T105.33-366q69.34-31.67 129.67-46.17 60.33-14.5 123.67-14.5 63.33 0 123.33 14.5T611.33-366q31 14.33 49.17 42.83T678.67-260v100h-640Zm706.66 0v-102.67q0-56.66-29.5-97.16t-79.16-66.84q63 7.34 118.66 22.5 55.67 15.17 94 35.5 34 19.34 53 46.17 19 26.83 19 59.83V-160h-176ZM249-524.33Q205.33-568 205.33-634T249-743.67q43.67-43.66 109.67-43.66t109.66 43.66Q512-700 512-634t-43.67 109.67q-43.66 43.66-109.66 43.66T249-524.33Zm439.33 0q-43.66 43.66-109.66 43.66-11 0-25.67-1.83-14.67-1.83-25.67-5.5 25-27.33 38.17-64.67Q578.67-590 578.67-634t-13.17-80q-13.17-36-38.17-66 12-3.67 25.67-5.5 13.67-1.83 25.67-1.83 66 0 109.66 43.66Q732-700 732-634t-43.67 109.67Zm-583 297.66H612V-260q0-14.33-8.17-27.33-8.16-13-20.5-18.67-66-30.33-117-42.17-51-11.83-107.66-11.83-56.67 0-108 11.83-51.34 11.84-117.34 42.17-12.33 5.67-20.16 18.67-7.84 13-7.84 27.33v33.33Zm315.17-345.5Q445.33-597 445.33-634t-24.83-61.83q-24.83-24.84-61.83-24.84t-61.84 24.84Q272-671 272-634t24.83 61.83q24.84 24.84 61.84 24.84t61.83-24.84Zm-61.83 345.5Zm0-407.33Z"/></svg>
                </div>
                <p class="text-[#003d7e] text-[1.8rem] font-bold">ทรัพยากรบุคคล</p>
            </div>
            <div class="bg-white w-full max-w-[380px] sm:w-[380px] h-[200px] rounded-[1rem] shadow-[0_2px_16px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-[0_6px_24px_rgba(40,74,126,0.15)]">
                <div class="logo w-[52px] h-[52px] bg-[#19528f] rounded-[0.75rem] mb-4 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 -960 960 960" width="28px" fill="#e3e3e3"><path d="M125-431.33q-1.67-12-2.5-24-.83-12-.83-24.67 0-75 28-140.5t76.66-114q48.67-48.5 114-77Q405.67-840 480-840q74.33 0 139.83 28.5 65.5 28.5 114.34 77 48.83 48.5 77 114Q839.33-555 839.33-480q0 12.67-.83 24.67-.83 12-2.5 24h-67.67q2-12 3.17-24t1.17-24.67q0-12-.84-24-.83-12-2.83-24H633.67q1 12 1.33 24 .33 12 .33 24 0 12.67-.33 24.33-.33 11.67-1.33 24.34H568q.67-10 .67-20.17v-20.17q0-15.33-.17-29.33-.17-14-1.83-27H394.33q-1.66 13-2.16 27-.5 14-.5 29.33v20.17q0 10.17.66 20.17h-65.66q-1-12-1.34-24-.33-12-.33-24.67 0-12 .33-24 .34-12 1.34-24H192q-2 12-2.83 24-.84 12-.84 24 0 12.67 1.17 24.67 1.17 12 3.17 24H125Zm86.33-163.34h123Q343-645 357.67-684.5 372.33-724 395-760q-62 18-110.33 61.5-48.34 43.5-73.34 103.83Zm192 0h153.34Q548-644.33 528.33-690q-19.66-45.67-48.33-86.67-28.67 41-48.5 86.67t-28.17 95.33Zm222.67 0h123.67q-25-60.33-73.84-103.83Q627-742 565-760q22.67 36.67 37.5 75.83 14.83 39.17 23.5 89.5ZM446.67-120v-48q0-54.67-37.67-92.33Q371.33-298 316.67-298H80v-66.67h236.67q49.33 0 91.83 22.34Q451-320 480-280.67q29-39.33 71.17-61.66 42.16-22.34 91.5-22.34H880V-298H642.67q-54 0-91.67 38t-37.67 92v48h-66.66Z"/></svg>
                </div>
                <p class="text-[#003d7e] text-[1.8rem] font-bold">ระบบสารสนเทศ</p>
            </div>
        </div>
    </section>
</template>

<script>
export default {
    name: "UhosnetGroup",
    
};
</script>

